...
```hcl
licenses = {
  "ip-10-0-1-100.us-east-1.compute.internal" = "/path/to/app-node-1.lic"
  "ip-10-0-2-200.us-east-1.compute.internal" = "/path/to/app-node-2.lic"
  "ip-10-0-3-300.us-east-1.compute.internal" = "/path/to/gpu-node.lic"
}
```
...
Disabled by default. Set `gpu_enabled = true` to add a single `g5.2xlarge` node on the `AL2023_x86_64_NVIDIA` AMI, tainted `fortiaigate-gpu=true:NoSchedule`. Triton runs there exclusively.
...
If `licenses` is set while GPU support is enabled, include a license entry for each app node plus the GPU node. Triton uses both GPU placement and licensed-node affinity. Get the GPU node name with:

```bash
kubectl get nodes -l fortiaigate-role=gpu -o custom-columns=NAME:.metadata.name --no-headers
```
...
| <a name="input_gpu_node_ami_type"></a> [gpu\_node\_ami\_type](#input\_gpu\_node\_ami\_type) | EKS managed node group AMI type for GPU nodes. Use AL2023\_x86\_64\_NVIDIA for CUDA 13+ container compatibility. | `string` | `"AL2023_x86_64_NVIDIA"` | no |
